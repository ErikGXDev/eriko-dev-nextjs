  ...
  return (
    <div className="grid grid-cols-1 gap-5 md:grid-cols-2 mt-8 mb-16">
      {projects.map((project: any) => (
        <a
          rel="noreferrer"
          className="transition-transform duration-120 p-4 border dark:border-zinc-700 hover:shadow-lg hover:-translate-y-1"
          target="_blank"
          href={project.link}
          key={project.name}
        >
          ...
          <div className="flex gap-1 items-center">
            <div className="font-semibold text-lg flex gap-1 items-center">
              {project.featured && (
                <StarFill className="h-4 w-4 fill-amber-500" />
              )}
              <h1>{project.name}</h1>
            </div>
            {!project.released && (
          ...
          <p className="text-[12px] mt-1 text-zinc-500 dark:text-zinc-400">
            {project.tools}
          </p>
        </a>
      ))}
